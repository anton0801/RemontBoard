...
final class AppDelegate: UIResponder, UIApplicationDelegate, MessagingDelegate, UNUserNotificationCenterDelegate {
    ...
    func messaging(_ messaging: Messaging, didReceiveRegistrationToken fcmToken: String?) {
        messaging.token { token, error in
            ...
            UserDefaults(suiteName: "group.remont.storage")?.set(token, forKey: "shared_fcm")
        }
    }
...
final class AttributionBridge: NSObject {
    ...
    func receiveNavigation(_ data: [AnyHashable: Any]) {
        guard !UserDefaults.standard.bool(forKey: "rb_first_launch_flag") else { return }
        navigationBuf = data
        onNavigation?(data)
        timer?.invalidate()
...
final class SDKBridge: NSObject, AppsFlyerLibDelegate, DeepLinkDelegate {
    ...
    func configure() {
        ...
        sdk.appsFlyerDevKey = RemontConfig.devKey
        sdk.appleAppID = RemontConfig.appID
        sdk.delegate = self
...
struct RemontConfig {
    static let appID = "6759557009"
    static let devKey = "mYdMQQgbVS42wiGywtbWr7"
}
